<center>
    <form class="contentBox" action="index.php?page=generator" method="post">
        <h2 class="boxTitle">Hash Generator</h2>
		<div class="boxContent">
				...
				<?php //Print hash result
					...
					if(isset($_POST["submitHashGen"])) {
						...
						if (empty($text)) {
							...
							if (!empty($hash)) {
								echo '<input type="text" name="text" class="form-control bg-dark" style="color:white;" aria-label="Small" aria-describedby="inputGroup-sizing-sm" value='.$hash.'>';
			...
			<div class="input-group input-group-sm mb-3 bg-dark">
				<div class="input-group-prepend bg-dark">
					<span class="input-group-text bg-dark" style="color:white;">Text</span>
				</div>
					<input type="text" name="text" class="form-control bg-dark" style="color:white;" aria-label="Small" aria-describedby="inputGroup-sizing-sm">
				</div>
				<div class="input-group mb-3">
					<div class="input-group-prepend">
						<label class="input-group-text bg-dark" style="color: white" for="inputGroupSelect01">Algorithm</label>
					</div>
					<select name="hashType" class="custom-select bg-dark" style="color: white">
			...
			<button class="inputButton bg-dark btn generateButton" name="submitHashGen">Generate Hash</button>
		</div>
	</form>
</center>