	...
	if (!$adminController->isUserOwner()) {
		...
		if (!empty($_GET["name"]) or !empty($_GET["editor"])) {
			echo '<ul class="breadcrumb bg-dark">';
				...
				if (!empty($_GET["name"])) {
					echo '<li><a class="selectorButton btn-small" href="?admin=dbBrowser&delete='.$_GET["name"].'&id=all"><strong>Delete all '.$_GET["name"].'</strong></a></li>';
				}
			...
			//Check if set editor for edit item by id
				...
				if (isset($_POST["submitEdit"])) {
					...
					if (isset($_GET["postby"]) and $_GET["postby"] == "todomanager") {
						...
						$urlUtils->jsRedirect("?admin=dbBrowser&name=".$_GET['editor']."&limit=".$limitOnPage."&startby=0");
					}
				}
				...
				if ($tableData->num_rows == 0) {
					...
					if ($_GET["name"] != "visitors" && $_GET["name"] != "pastes" && $_GET["name"] != "hash_gen" && $_GET["name"] != "users") {
